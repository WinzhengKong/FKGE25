...
class Server:
    ...
    def aggregate_embedding(self,entity_embedding_dict,byzantine_client_indices):
        ...
        for c_idx in range(0, args.client_num):
            distance = detect_attack_in_embeddings(entity_embedding_dict[c_idx].cpu().detach().numpy())
        ...
        if args.agg == "FedAD":
            ...
            if args.adm == "ECOD":
                ...
                mean_value = np.mean(client_average_scores)
                processed_scores = np.where(client_average_scores < mean_value, 1, 0)
            ...
            for client_seq in entity_embedding_dict.keys():
                if args.agg=="distance":
                    ...
                    if processed_scores[client_seq] > 0:
                        ...
                        '''
                        print(f'byzantine client embedding {client_seq} is excluded!')
                        '''
